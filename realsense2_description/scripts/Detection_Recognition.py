...
SCORE_THRESHOLD = 0.5
IOU_THRESHOLD = 0.5
config_path = "/home/cyr-cesar/ur5_project/realsense_ws/src/realsense2_description/scripts/yolov2.cfg"
weights_path = "/home/cyr-cesar/ur5_project/realsense_ws/src/realsense2_description/scripts/yolov2.weights"
font_scale = 1
thickness = 1

LABELS = open('/home/cyr-cesar/ur5_project/realsense_ws/src/realsense2_description/scripts/coco.names').read().strip().split("\n")
...
def check_distance(x,y,w,h):
    ...
    ch = int(h)
    print(x, y)
    if((depth_image[cy,cx]/10) <60 and (cx+cw<640 and (depth_image[cy,cx+cw]/10)<60) and (cy+ch<480 and (depth_image[cy+ch,cx]/10)<60)
                and (depth_image[cy+ch,cx+cw]/10)<60 and (depth_image[cy+int(h/2),cx+int(cw/2)]/10)<60):
                    return True
    else:
        return False
...
def update_object_map_from_file(object_pose):
        ...
        fx = new_msg_pose.orientation.x
        fy = new_msg_pose.orientation.y
        fz = new_msg_pose.orientation.x
...
#rospy.Subscriber('/gazebo/performance_metrics', Odometry, get_odmetry)
